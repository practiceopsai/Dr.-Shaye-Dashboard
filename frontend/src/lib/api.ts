...
export type PhoneJob = {id:string;transcript:string;state:string;created:number;result:string;error:string;callback_requested:number;question?:string;resume_job?:string;parent_id?:string;cancel_requested?:number|null;actions?:{event_id:string;status:string;content:string}[]};
...
export const api={
  ...
  cancelPhoneTask:(id:string)=>call<{state:string;effect_cancelled:boolean}>(`/api/phone/jobs/${encodeURIComponent(id)}/cancel`,{method:"POST"}),
