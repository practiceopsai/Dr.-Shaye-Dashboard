...
export function Composer({ api, owner, card, mode, online, onSent, close }: {
  ...
  const save = (value: string) => drafts.save(owner, kind, value).catch(() => {
  ...
  useEffect(() => {
    ...
    const sub = AppState.addEventListener('change', state => {
      if (state !== 'active') { ExpoSpeechRecognitionModule.abort(); void save(currentText.current); }
    });
  ...
  async function saveAndClose() { await save(currentText.current); close(); }
  ...
  return <View style={s.gap}>
    ...
    {!!receipt ? <><Notice>{receipt}</Notice><Button label="Done" onPress={close} /></> : <>
      ...
      <View style={s.between}><Small>{text.length}/{limit}</Small><Small>Draft stored securely on this phone</Small></View>
      ...
      <Button secondary label="Save draft and close" disabled={busy} onPress={() => { void saveAndClose(); }} />
    </>}
  </View>;
}
