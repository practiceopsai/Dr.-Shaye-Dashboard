...
Build a React Native application with Expo development builds. Reuse the existing FastAPI backend and its typed dashboard contract. Build native Today, Schedule, Commitments, Decisions, Eli status, and request/feedback screens. The phone must not contain Orgo, Anthropic, Composio, or AgentMail secrets. There is one production Eli vault and one set of priorities for both authorized users.
...
| Existing compatible JavaScript/assets | Optional signed EAS Update within the approved native runtime and Apple's rules; activate on a safe restart, preserving drafts |
...
## Inputs needed before signing and distribution

- Apple Developer membership and the owning team; App Store Connect access.
- Bundle identifier and iOS OAuth configuration in the existing Google Cloud project.
- Expo/EAS ownership if cloud builds and signed compatible updates are used.
- The two tester Apple account emails and approval to send those invitations.

These inputs do not block dashboard/backend work. No Apple membership purchase, app submission, or tester invitation has been made by this plan. TestFlight builds expire after 90 days, so release maintenance is required even during a private beta.
